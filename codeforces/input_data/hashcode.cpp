...
int32_t main(){
	...
	for(int i=0 ; i<n ; i++){
		...
		for(int j=1 ; j<=sz ; j++){
            ...
            mm[persons[i].name].push_back(curr_skill);
        ...
        for(auto rskill: projects[i].reqd_skills){
            ...
            for(auto &pp: m[rskill.course]){
                ...
                if(pp.avalilable<projects[i].deadline-projects[i].duration && rskill.level<=k){
                    ...
                    //     for(auto sss: temp_2){
                    //         for(auto sk :mm[sss]){
                    //             if(sk.course==rskill.course && sk.level>=rskill.level){
                    //                 has = true;
                    //             }
                    //         }
                    //     }
                    //     if(it==temp_2.end() && has){
                    ...
                    // else{
                        ...
                        if(it==temp_2.end()){
                            pp.avalilable = projects[i].deadline;
                            for(int ii=0; ii<pp.skills.size(); ii++){
                                if(rskill.level==k && pp.skills[ii].course==rskill.course){
                                    pp.skills[ii].level++;
                                }
                            }
                            temp_2.push_back(pp.name);
